        ...
        if "params.json" in files:
            ...
            try:
                ...
                if isinstance(lr, list):
                    ...
                    if start == 0.0003: start_s = "3e-4"
                    elif start == 0.0001: start_s = "1e-4"
                    else: start_s = str(start)
                    exp_data['Learning Rate'] = f"{start_s} -> {end}"
                else:
                    if lr == 0.0003: lr_s = "3e-4"
                    else: lr_s = str(lr)
                    ...
                    with open(res_path, 'r') as rf:
                        for line in rf:
                            try:
                                ...
                                # Use iteration with Best Mean as the "Peak" snapshot
                                # OR Global Max? 
                                # Reliability is usually "How stable is it when it's performing well?"
                                # So I'll take Min/Max *at the moment of Peak Mean*? 
                                # No, usually Ray logs stats *for that iteration*.
                                # So "Range at Peak" is correct.
                ...
                if p_mean != -np.inf and p_max != -np.inf and p_min != -np.inf:
                    exp_data['Peak Mean'] = f"{p_mean:.1f}"
                    exp_data['Peak Min'] = f"{p_min:.1f}"
                    exp_data['Peak Max'] = f"{p_max:.1f}"
                    # Range
                    diff = p_max - p_min
                    exp_data['Range (Max-Min)'] = f"{diff:.1f}"
                else:
                    exp_data['Peak Mean'] = "-"
                    exp_data['Peak Min'] = "-"
                    exp_data['Peak Max'] = "-"
                    exp_data['Range (Max-Min)'] = "-"
...
if not df.empty:
    ...
    # Cols reordered for reliability emphasis
    cols = ['Date', 'ID', 'Network Architecture', 'Batch Size', 'MiniBatch Size', 'Epochs', 'Learning Rate', 'Entropy Coefficient', 'Initialization', 'Activation', 'Peak Mean', 'Peak Min', 'Peak Max', 'Range (Max-Min)']
